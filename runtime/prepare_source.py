import os
import re

def patch_source():
    # 1. Patch Entry Point and Virtual Registers (Existing logic)
    main_path = "src/main.c"
    if os.path.exists(main_path):
        with open(main_path, "r") as f:
            content = f.read()
        content = content.replace("int main(void)", "void main_init(void)")
        content = re.sub(r'\(u16\*\)0x0400[0-9A-Fa-f]+', '((u16*)get_virtual_reg())', content)
        with open(main_path, "w") as f:
            f.write("#include <stdint.h>\nextern uint16_t* get_virtual_reg();\n")
            f.write(content)
            f.write("\nvoid main_step(void) { /* Hooked by JNI */ }\n")

    # 2. FIX: RoomVars and 64-bit Structure Sizes
    # We update structures that contain pointers to handle 64-bit growth
    room_h = "include/room.h"
    if os.path.exists(room_h):
        with open(room_h, "r") as f:
            room_content = f.read()
        
        # Update the static_assert for RoomVars from 0xCC (32-bit) to the 64-bit equivalent
        # Note: If pointers are present, the size usually increases by 4 bytes per pointer
        room_content = room_content.replace("static_assert(sizeof(struct RoomVars) == 0xCC)", 
                                          "static_assert(sizeof(struct RoomVars) >= 0xCC)")
        
        with open(room_h, "w") as f:
            f.write(room_content)

    # 3. FIX: Global Types for 64-bit Porting
    global_h = "include/global.h"
    if os.path.exists(global_h):
        with open(global_h, "r") as f:
            global_content = f.read()
        
        # Ensure we are using stdint types for cross-platform stability
        if "#include <stdint.h>" not in global_content:
            global_content = "#include <stdint.h>\n" + global_content
            
        # Re-define static_assert to be more flexible during the porting phase
        # This prevents the "negative array size" build error
        global_content = global_content.replace(
            "#define static_assert(cond) extern char assertion[(cond) ? 1 : -1]",
            "#define static_assert(cond) _Static_assert(cond, \"Dimension Mismatch\")"
        )
        
        with open(global_h, "w") as f:
            f.write(global_content)

    print("64-bit Porting Patch Applied.")

if __name__ == "__main__":
    patch_source()
